...
def prepare(df):
    ...
    if 'ou25' not in x and {'home_goals','away_goals'}.issubset(x.columns): x['ou25']=((x.home_goals+x.away_goals)>2.5).astype(int)
    if 'btts' not in x and {'home_goals','away_goals'}.issubset(x.columns): x['btts']=((x.home_goals>0)&(x.away_goals>0)).astype(int)
    return x
